...
def pipeline_workdir():
    ...
    try:
        ...
        step3()
        _reset()
        optimize_step4(optimization_method="dominating-set", verbose=False)
    except Exception:
...
class TestPipelineE2E:
    ...
    def test_step2_primers_are_valid_dna(self, pipeline_workdir):
        ...
        for primer in df["primer"].head(20):
            assert set(primer.upper()).issubset(
                valid_bases
    ...
    def test_full_pipeline_runs_sequentially(self, pipeline_workdir, reset_pipeline_state):
        ...
        assert len(primer_sets) > 0 and len(primer_sets[0]) > 0, "Step 4 produced no primers"
