...
def test_schema_passes_plasmid_example_params():
    ...
    assert not errors, "plasmid_example params.json should match the schema; got: " + "; ".join(
...
def test_param_validator_flags_schema_violation():
    ...
    assert any(
        "polymerase" in m.parameter for m in errors
...
def test_schema_cli_output_writes_file(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "neoswga.cli_unified", "schema", "--dump", "--output", str(out)],
        capture_output=True,
        text=True,
        timeout=30,
    )
