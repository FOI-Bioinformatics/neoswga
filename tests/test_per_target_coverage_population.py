...
def test_hybrid_populates_per_target_coverage(primed_workdir):
    ...
    assert per_target, f"per_target_coverage should be populated after optimize; got {per_target!r}"
    # Plasmid example has one fg prefix (pcDNA)
    assert "pcDNA" in {
        str(k).split("/")[-1] for k in per_target
    }, f"expected pcDNA key in per_target_coverage; got {list(per_target)}"
    for prefix, cov in per_target.items():
        assert 0.0 <= cov <= 1.0, f"per_target coverage for {prefix} out of range: {cov}"
