...
class TestBlacklistCLIArguments:
    ...
    def test_count_kmers_blacklist_args(self):
        ...
        args = parser.parse_args([
            'count-kmers', '-j', 'params.json',
            '--blacklist', 'genome1.fna', 'genome2.fna',
            '--bl-penalty', '3.0',
            '--max-bl-freq', '0.001'
        ])
    ...
    def test_filter_blacklist_args(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'filter', '-j', 'params.json',
            '--blacklist', 'contaminant.fna',
        ])
    ...
    def test_init_blacklist_args(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'init', '--genome', 'target.fna',
            '--blacklist', 'contaminant1.fna', 'contaminant2.fna',
        ])
    ...
    def test_genome_add_args(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'genome-add', 'human-grch38', 'human.fna',
            '--role', 'bg', '--k-ranges', '6-12',
        ])
