...
def _load_report_template() -> str:
    """Load the technical-report HTML template (uses str.format placeholders).

    The template lives in neoswga/core/report/templates/technical_report.html
    (a package data file) instead of inline, so it can be edited and linted as
    HTML. Cached after first read.
    """
    from importlib.resources import files

    return (files("neoswga.core.report") / "templates" / "technical_report.html").read_text(
        encoding="utf-8"
    )
...
"""
    ...
    # Render all sections
    html = _load_report_template().format(
        # Colors
        **colors,
        # Header
