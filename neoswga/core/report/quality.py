...
def _generate_recommendation(
    ...
    for comp in weak_components:
        if comp.name == "Coverage":
            ...
            considerations.append(
                f"Tm range ({comp.raw_value:.1f}C) is wide. " "Consider tighter Tm filtering."
            )
        elif comp.name == "Dimer Risk":
            considerations.append(
                "Primer-dimer interactions detected. " "Monitor amplification efficiency."
            )
...
def calculate_quality_grade(metrics: PipelineMetrics) -> QualityAssessment:
    ...
    short_rec, detailed_rec, considerations = _generate_recommendation(grade, components)
