...
def get_all_files(prefix_or_file):
    ...
    for individual_prefix in prefix_or_file:
        list_of_files = os.listdir(
            os.path.dirname(individual_prefix)
...
def get_params(args):
    ...
    max_dimer_bp = data["max_dimer_bp"] = get_value_or_default(
        args.max_dimer_bp, data, "max_dimer_bp"
    )
    max_self_dimer_bp = data["max_self_dimer_bp"] = get_value_or_default(
        args.max_self_dimer_bp, data, "max_self_dimer_bp"
    )
    verbose = data["verbose"] = get_value_or_default(args.verbose, data, "verbose")

    drop_iterations = data["drop_iterations"] = get_value_or_default(
        args.drop_iterations, data, "drop_iterations"
    )
    iterations = data["iterations"] = get_value_or_default(args.iterations, data, "iterations")
    top_set_count = data["top_set_count"] = get_value_or_default(
        args.top_set_count, data, "top_set_count"
    )
    ...
    selection_metric = data["selection_metric"] = get_value_or_default(
        args.selection_metric, data, "selection_metric"
    )
    ...
    min_k = data["min_k"] = get_value_or_default(
        args.min_k if hasattr(args, "min_k") else None, data, "min_k"
    )
    max_k = data["max_k"] = get_value_or_default(
        args.max_k if hasattr(args, "max_k") else None, data, "max_k"
    )
    ...
    kmer_counter = data.get("kmer_counter", "kmc")
    ...
    if args.kmer_fore is not None:
        if len(data["fg_genomes"]) > 1:
            data["fg_prefixes"] = [
                args.kmer_fore + os.path.splitext(os.path.basename(genome_fname))[0]
                for genome_fname in data["fg_genomes"]
            ]
        else:
    ...
    if args.kmer_back is not None:
        if len(data["bg_genomes"]) > 1:
            data["bg_prefixes"] = [
                args.kmer_back + "_" + os.path.splitext(os.path.basename(genome_fname))[0]
                for genome_fname in data["bg_genomes"]
            ]
        else:
    ...
    if "fg_genomes" in data and (
        "fg_seq_lengths" not in data or len(data["fg_seq_lengths"]) != len(data["fg_genomes"])
    ):
        data["fg_seq_lengths"] = _utility.get_all_seq_lengths(
            fname_genomes=data["fg_genomes"], cpus=data["cpus"]
        )
    ...
    if use_bloom_filter and len(data.get("bg_prefixes", [])) == 0:
        data["bg_seq_lengths"] = []
    elif "bg_genomes" in data and (
        "bg_seq_lengths" not in data or len(data["bg_seq_lengths"]) != len(data["bg_genomes"])
    ):
    ...
    if "bl_genomes" in data and data["bl_genomes"]:
        if "bl_seq_lengths" not in data or len(data.get("bl_seq_lengths", [])) != len(
            data["bl_genomes"]
        ):
