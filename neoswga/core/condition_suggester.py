...
class ConditionSuggester:
    ...
    def suggest(self) -> ConditionRecommendation:
        ...
        rationale.append(
            f"Primer length: {primer_range[0]}-{primer_range[1]}bp "
    ...
    def _select_polymerase(self) -> Tuple[str, str]:
        ...
        if self.gc_class in ("extreme_at", "at_rich"):
            if self.primer_length and self.primer_length > 12:
                return (
                    "equiphi29",
                    f"Polymerase: EquiPhi29 (longer primers for {self.gc_class.replace('_', ' ')} genome)",
                )
            return (
                "phi29",
                f"Polymerase: Phi29 (simpler for {self.gc_class.replace('_', ' ')} genome)",
            )

        elif self.gc_class in ("gc_rich", "extreme_gc"):
            return (
                "equiphi29",
                f"Polymerase: EquiPhi29 (better GC tolerance for {self.gc_class.replace('_', ' ')} genome)",
            )
            ...
            if self.primer_length and self.primer_length > 14:
                return (
                    "equiphi29",
                    "Polymerase: EquiPhi29 (longer primers benefit from higher temperature)",
                )
    ...
    def _suggest_primer_range(self, polymerase: str) -> Tuple[int, int]:
        ...
        if self.primer_length:
            ...
            margin = 2
            return (max(6, self.primer_length - margin), min(20, self.primer_length + margin))
        ...
        if rec.alternatives:
            ...
            for alt in rec.alternatives:
                if "polymerase" in alt:
                    print(
                        f"  - {alt['polymerase'].upper()} at {alt['temp']}C: {alt['description']}"
                    )
                elif "method" in alt:
...
def suggest_conditions(
    ...
    ctx = (
        ...
        if context in [e.value for e in ApplicationContext]
        else ApplicationContext.STANDARD
    )

    suggester = ConditionSuggester(genome_gc=genome_gc, primer_length=primer_length, context=ctx)
...
def sweep_conditions(
    ...
    if verbose and results:
        ...
        print("=" * 80)
        print(
            f"{'Rank':<5} {'Temp':>5} {'DMSO%':>6} {'Bet.M':>6} {'Mg2+':>5} "
            f"{'Ampl.':>7} {'Proc.':>6} {'Access':>7} {'Eff.Tm':>7}"
        )
        print("-" * 80)
        for i, r in enumerate(results[:15], 1):
            print(
                f"{i:<5} {r['temperature']:>5.1f} {r['dmso_percent']:>6.1f} "
