...
class CliqueOptimizerConfig(OptimizerConfig):
    ...
    # Maximum number of dimer-free sets to enumerate
...
def build_compatibility_graph(
    ...
    logger.info(
        f"Compatibility graph: {len(valid_primers)}/{n} primers " f"passed self-dimer check"
    )
    ...
    logger.info(
        f"Compatibility graph: {edge_count}/{total_pairs} edges " f"(density={density:.2f})"
    )
...
def enumerate_dimer_free_sets(
    ...
    for clique in nx.find_cliques(G):
        ...
        if len(clique) == target_size:
            ...
            # Extract target_size subsets from larger cliques.
            #
            # The cap has to be tested INSIDE this loop. Tested only after it,
            # a single large clique ran `combinations` to exhaustion first --
            # a clique of 50 at target 6 is 15.8 million subsets, each one
            # appended to a list and a set before anything checked the limit.
            # The cap was there to bound the work and instead bounded only the
            # number of cliques that got to blow up.
        ...
        if len(sets) >= max_cliques:
            logger.info(
                f"Reached max_cliques limit ({max_cliques}); enumeration "
                f"stopped early, so the search was NOT exhaustive."
            )
            break
    ...
    def optimize(
        ...
        **kwargs,
                ...
                for j in range(i + 1, len(fixed)):
                    if is_dimer_fast(fixed[i], fixed[j], max_dimer_bp):
                        return OptimizationResult.failure(
                            self.name, f"Fixed primers {fixed[i]} and {fixed[j]} " f"form a dimer"
                        )
        ...
        if len(candidates) > self.clique_config.max_pool_size:
            ...
            candidates = self._rank_and_truncate(candidates, self.clique_config.max_pool_size)

        if len(candidates) < target:
            return OptimizationResult.failure(
                self.name,
                f"Not enough candidates ({len(candidates)}) for " f"target size ({target})",
            )

        logger.info(f"Clique optimizer: {len(candidates)} candidates, " f"target size {target}")
        ...
        G = build_compatibility_graph(
            ...
            max_self_dimer_bp=getattr(self.config, "max_self_dimer_bp", max_dimer_bp + 1),
        ...
        if fixed:
            ...
            for fp in fixed:
                if fp in G:
                    ...
                    logger.warning(
                        f"Fixed primer {fp} excluded from graph " f"(possible self-dimer)"
                    )
            G = G.subgraph(compatible_nodes).copy()
            logger.info(f"After fixed primer filtering: {len(G.nodes())} " f"compatible candidates")
        ...
        logger.info(f"Enumerated {len(dimer_free_sets)} dimer-free sets " f"in {enum_time:.1f}s")
        ...
        #
        # Two stages, because the two costs are very different. `_score_set` is
        # a site-count ratio needing only cached positions, so it can rank
        # thousands of sets; `compute_metrics` walks the genome base by base and
        # cannot. Ranking cheaply and then scoring the shortlist properly keeps
        # the selection objective identical to the one every other optimizer
        # reports -- `normalized_score`, the only value comparable across
        # optimizers. Selecting on the raw ratio instead would have made this
        # optimizer pick for site density while its own result was judged, and
        # its PARTIAL status set, on coverage.
        ranked = sorted(
            (fixed + primer_set for primer_set in dimer_free_sets),
            key=self._score_set,
            reverse=True,
        )
        shortlist = ranked[: self.clique_config.max_scored_sets]
        ...
        if len(ranked) > len(shortlist):
            logger.info(
                f"Scored the top {len(shortlist)} of {len(ranked)} dimer-free "
                f"sets by binding ratio; the rest were not evaluated."
            )
