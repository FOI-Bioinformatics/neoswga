...
class AmplificationNetwork:
    ...
    def _rebuild_spatial_index(self):
        ...
        sorted_pairs = sorted([(s.position, s) for s in self.binding_sites], key=lambda x: x[0])
    ...
    def add_primer_sites(
        ...
        for pos in positions:
            site = BindingSite(position=int(pos), strand=strand, primer=primer, affinity=affinity)
    ...
    def add_edges_for_sites(self, new_sites: List[BindingSite]) -> int:
        ...
        for new_site in new_sites:
            ...
            nearby_sites = self._find_sites_in_range(new_site.position, self.max_extension)
            ...
            for other_site in nearby_sites:
                ...
                edge = AmplificationEdge(
                    source=new_site, target=other_site, distance=distance, feasible=True
                )
        ...
        for i, site1 in enumerate(self._sorted_sites):
            ...
            for j in range(i + 1, end_idx):
                ...
                if site1.strand != site2.strand:
                    ...
                    edge = AmplificationEdge(
                        source=site1, target=site2, distance=distance, feasible=True
                    )
...
class NetworkOptimizer:
    ...
    def optimize_greedy(self, candidates: List[str], num_primers: int = 10) -> List[str]:
        ...
        if self.mech_model is not None:
            logger.info(
                f"  Mechanistic weight: {self.mechanistic_weight:.2f} (template GC: {self.template_gc:.2f})"
            )
        ...
        for iteration in range(num_primers):
            ...
            for primer in candidates:
                ...
                if self.mech_model is not None and self.mechanistic_weight > 0:
                    ...
                    score = score * (
                        1.0 - self.mechanistic_weight + self.mechanistic_weight * mech_factor
                    )
        ...
        logger.info(
            f"Final target network: {fg_stats['largest_component']} largest component, "
            f"{fg_stats['predicted_amplification']:.0f}× predicted amplification"
        )
        logger.info(
            f"Final background network: {bg_stats['avg_component_size']:.1f} avg component size"
        )
    ...
    def _evaluate_primer_addition(
        ...
        old_cv = (
            fg_network.coverage_uniformity(genome_length)
            if fg_network.binding_sites
            else float("inf")
        )
        ...
        # Combine scores
        final_score = (
            1.0 - uniformity_weight
        ) * base_score + uniformity_weight * uniformity_improvement
    ...
    def score_primer_set(
        ...
        mech_metrics = {}
        if self.mech_model is not None:
            effects_list = [self.mech_model.calculate_effects(p, self.template_gc) for p in primers]
            if effects_list:
                import numpy as np

                mech_metrics = {
                    "avg_processivity_factor": float(
                        np.mean([e.processivity_factor for e in effects_list])
                    ),
                    "avg_accessibility": float(
                        np.mean([e.accessibility_factor for e in effects_list])
                    ),
                    "avg_amplification_factor": float(
                        np.mean([e.predicted_amplification_factor for e in effects_list])
                    ),
                    "min_amplification_factor": float(
                        min(e.predicted_amplification_factor for e in effects_list)
                    ),
                    "avg_effective_binding": float(
                        np.mean([e.effective_binding_rate for e in effects_list])
                    ),
                    "avg_stability_factor": float(
                        np.mean([e.stability_factor for e in effects_list])
                    ),
                }
        ...
        if self.cache is not None and hasattr(self.cache, "compute_strand_alternation_stats"):
            for prefix, length in zip(self.fg_prefixes, self.fg_seq_lengths):
                try:
                    ...
                    result["strand_alternation_gap_mean"] = strand_stats[
...
def benchmark_network_vs_ratio(
    position_cache, fg_prefixes, bg_prefixes, fg_seq_lengths, bg_seq_lengths, candidates
):
    """
