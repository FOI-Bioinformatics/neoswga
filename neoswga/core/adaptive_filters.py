...
class FilterThresholds:
    ...
    def __str__(self):
        return (
            f"GC: [{self.gc_min:.3f}, {self.gc_max:.3f}], "
    ...
    def filter_primers(self, candidates: List[str], verbose: bool = False) -> List[str]:
        ...
        for primer in candidates:
            ...
            if not failed:
                ...
                reasons = [
                    f"{name}: {self.filters[name].explain_rejection(primer)}" for name in failed
                ]
                logger.debug(f"{primer} rejected: {'; '.join(reasons)}")

        logger.info(
            f"Adaptive filtering: {len(candidates)} → {len(passed)} "
...
def run_step2_with_adaptive_gc(gc_tolerance: float = 0.15):
    ...
    if not fg_genomes or len(fg_genomes) == 0:
        raise ValueError(
            "No target genome specified in params.json. " "Cannot compute adaptive GC thresholds."
        )
