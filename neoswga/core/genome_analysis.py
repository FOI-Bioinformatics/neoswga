...
def analyze_genome(
    ...
    for record in SeqIO.parse(genome_path, "fasta"):
        ...
        contigs.append(
            {"id": record.id, "length": contig_length, "gc": contig_gc, "n_count": contig_n}
        )
    ...
    results["parameter_recommendations"] = {
        ...
        "gc_bounds": {"min": max(0.0, gc_content - 0.15), "max": min(1.0, gc_content + 0.15)},
...
def _get_suitability_factors(results: Dict) -> list:
    ...
    if gc_class == "balanced":
        factors.append(
            {"factor": "GC content", "impact": "positive", "note": "Balanced GC is optimal"}
        )
    elif gc_class in ("at_rich", "gc_rich"):
        factors.append(
            {"factor": "GC content", "impact": "neutral", "note": "May need adjusted parameters"}
        )
    else:
        factors.append(
            {
                "factor": "GC content",
                "impact": "negative",
                "note": "Extreme GC requires special handling",
            }
        )

    if results["gc_analysis"]["gc_std"] < 0.05:
        factors.append(
            {"factor": "GC uniformity", "impact": "positive", "note": "Low GC variability"}
        )
    elif results["gc_analysis"]["gc_std"] > 0.10:
        factors.append(
            {"factor": "GC uniformity", "impact": "negative", "note": "High GC variability"}
        )
...
def _write_text_summary(results: Dict, output_path: Path) -> None:
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"GC Range: {results['gc_analysis']['gc_min']:.1%} - {results['gc_analysis']['gc_max']:.1%}\n\n"
        )
