...
class MechanisticModel:
    ...
    def _calculate_gc_normalization(self, gc_content: float, primer_length: int) -> float:
        """
        ...
        betaine_factor = self._sigmoid(
            c.betaine_m, p["betaine_gc_midpoint"], p["betaine_gc_steepness"]
        )
    ...
    def _calculate_accessibility(self, template_gc: float) -> float:
        ...
        dmso_melt = p["dmso_max_effect"] * (1.0 - math.exp(-p["dmso_melt_rate"] * c.dmso_percent))
        ...
        if c.dmso_percent > 2.0 and c.formamide_percent > 1.0:
            ...
            combined_reduction = antagonism * (c.dmso_percent - 2.0) * (c.formamide_percent - 1.0)
    ...
    def _calculate_base_enzyme_activity(self) -> Dict[str, float]:
        ...
        if dmso <= poly["dmso_threshold"]:
            ...
            dmso_effect = poly["dmso_mild_coef"] * poly["dmso_threshold"] + poly[
                "dmso_steep_coef"
            ] * (dmso - poly["dmso_threshold"])
    ...
    def get_interaction_report(self, template_gc: float = 0.5) -> str:
        ...
        """
        return self._interaction_registry.get_interaction_report(self.conditions, template_gc)
