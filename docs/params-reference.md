...
| `coverage_geometry` | string | one of: symmetric, directional | - | Where a binding site reaches when coverage is computed. 'symmetric' credits [pos - reach, pos + reach); 'directional' credits the one way the polymerase can extend. Default 'symmetric', which every recorded figure was produced under. The reach was fitted alongside it, so changing this without coverage_reach recalibrates silently. |
...
| `kmer_counter` | string | one of: kmc, jellyfish | - | Which k-mer counter to run. Unset: KMC3 when installed, otherwise jellyfish, which produce identical tables. Set explicitly, the named counter is required and its absence is an error. |
