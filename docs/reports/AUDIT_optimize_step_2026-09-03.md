...
## F0 (CRITICAL) The `score` stage did not change the delivered set -- RESOLVED 2026-09-05

The finding that opened this audit, and the last one to close. Decision: retire
the amplification model from the default pipeline.

**The evidence.** The `min_amp_pred` gate removed 7 of 1222 candidates on the
S. aureus panel, 0 of 449 on E. coli and 0 of 319 on M. tuberculosis -- the
scores cluster well above the default threshold of 10.0, so it passed
essentially everything. And every step-4 consumer reads the primer column and
nothing else:
...
So a prediction was computed for every candidate and discarded before selection.
Asked directly whether it identified good primers, taking the top half of a pool
by `amp_pred` and optimizing over it produced the WORST of five half-pools,
behind all three random halves and behind the bottom half by the same measure.

The model compounds it. It is fit to synthetic data generated by a hand-written
rule in `scripts/retrain_rf_model.py`, so it reproduces an opinion rather than
measured amplification; under the default `fast_score` the delta-G features are
zeroed, leaving the prediction a pure function of the primer sequence, blind to
the genome and the reaction. Finding 22 above found the feature it leans on
hardest was mis-scaled as well.

**What was done.** `score` now carries candidates forward rather than scoring
them: it writes `step3_df.csv` with the step-2 measurements in the deterministic
order `order_step3_rows` sets, and loads no model. The stage is not deleted --
`step3_df.csv` is a required intermediate six modules read, and the order it
writes reaches an order-sensitive optimizer.

**Measured effect: none on any delivered panel.** Re-running the E. coli design
after the change returned an identical 160-primer set with identical coverage,
which is the finding restated as a result. The candidate order is byte-identical
to the model path's. What changes is cost: 0.2 s against 3.1 s on 449
candidates, and 5 columns written instead of 61.

**What is kept.** `--amp-model` restores the score column and its gate.
`score --enable-qa` now ranks on the QA score alone, since there is no RF half
to blend; with `--amp-model` the 0.7/0.3 blend returns. `min_amp_pred` set to a
non-default value without `--amp-model` warns, so the retirement does not create
another accepted-and-inert option -- this repository's recurring defect.

Pinned by `tests/test_score_stage_is_retired.py`.
